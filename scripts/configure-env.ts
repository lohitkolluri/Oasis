...
const PROMPTS: { key: string; label: string; secret?: boolean }[] = [
  ...
  { key: "NEXT_PUBLIC_RAZORPAY_KEY_ID", label: "Razorpay Key Id (rzp_test_... — test mode only)", secret: false },
  { key: "RAZORPAY_KEY_SECRET", label: "Razorpay Key Secret (test mode)", secret: true },
  { key: "RAZORPAY_WEBHOOK_SECRET", label: "Razorpay webhook secret (Dashboard → Webhooks)", secret: true },
