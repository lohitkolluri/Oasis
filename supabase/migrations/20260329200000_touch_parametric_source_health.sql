-- Atomic per-source health updates (fixes lost updates when many zones probe the same source_id in parallel)
...
REVOKE ALL ON FUNCTION touch_parametric_source_health(TEXT, BOOLEAN, INT, TIMESTAMPTZ, BOOLEAN, TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION touch_parametric_source_health(TEXT, BOOLEAN, INT, TIMESTAMPTZ, BOOLEAN, TEXT) TO service_role;

COMMENT ON FUNCTION touch_parametric_source_health IS
  'Row-locked merge for parametric_source_health; safe under concurrent adjudicator zone batches';
