...
const LLM_MODEL = 'arcee-ai/trinity-large-preview:free:free';
...
function checkWeatherMismatch(raw: Record<string, unknown> | null): boolean {
  ...
  if (t === 'severe_aqi') {
    ...
    const currentAqi =
      (raw.current_aqi as number | undefined) ??
      ((raw.hourly as { us_aqi?: (number | null)[] } | undefined)?.us_aqi ?? []).find(
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    for (const zone of zones) {
      ...
      if (heat3h)
        candidates.push({
          type: 'weather',
          severity: 8,
          geofence: { type: 'circle', lat: LAT, lng: LNG, radius_km: 5 },
          raw: heatRaw,
        });
      if (tomorrowKey && precip >= 4)
        candidates.push({
          ...
          severity: 7,
          geofence: { type: 'circle', lat: LAT, lng: LNG, radius_km: 5 },
          raw: { ...precipRaw, trigger: 'heavy_rain' },
        });
        ...
        if (historicalRes.ok && currentRes.ok) {
          ...
          if (historicalValues.length >= 48) {
            ...
            baselineMean = Math.round(
              historicalValues.reduce((s: number, v: number) => s + v, 0) / historicalValues.length,
            );

            isChronic = baseline75 >= 200;

            if (isChronic) {
              adaptiveThreshold = Math.min(500, Math.max(350, Math.round(baseline90 * 1.15)));
            } else {
              adaptiveThreshold = Math.min(500, Math.max(200, Math.round(baseline75 * 1.3)));
            }
          }

          if (currentAqi >= adaptiveThreshold) {
            const refBaseline = isChronic ? baseline90 : baseline75;
            const excessRatio = refBaseline > 0 ? (currentAqi - refBaseline) / refBaseline : 0;
            ...
            candidates.push({
              ...
              raw: {
                ...
                excess_percent: Math.round(
                  ((currentAqi - refBaseline) / Math.max(1, refBaseline)) * 100,
                ),
      ...
      if (newsDataKey && openRouterKey) {
        const trafficRes = await fetch(
          `https://newsdata.io/api/1/news?apikey=${newsDataKey}&q=traffic%20OR%20gridlock%20OR%20road%20closure&country=in&language=en&limit=3`,
        );
        if (trafficRes.ok) {
            ...
            const llmRes = await fetch(OPENROUTER_URL, {
              ...
              body: JSON.stringify({
                ...
                messages: [
                  {
                    role: 'user',
                    content: `Do any of these headlines indicate severe traffic gridlock affecting delivery work in India? Reply JSON only: {"qualifies":true/false,"severity":0-10}. Headlines: ${articles.map((a: { title?: string }) => a.title).join('; ')}`,
                  },
                ],
              }),
            });
                ...
                try {
                  ...
                  if (p.qualifies && (p.severity ?? 0) >= 6) {
                    candidates.push({
                      ...
                      geofence: { type: 'circle', lat: LAT, lng: LNG, radius_km: 10 },
                      raw: { articles, llm: p, trigger: 'traffic_gridlock' },
                    });
                  }
                } catch {
                  /* skip */
            ...
            const llmRes = await fetch(OPENROUTER_URL, {
              ...
              body: JSON.stringify({
                ...
                messages: [
                  {
                    role: 'user',
                    content: `Do any of these headlines indicate a zone lockdown/curfew/strike in India? Reply JSON only: {"qualifies":true/false,"severity":0-10,"zone":"city if identifiable"}. Headlines: ${articles.map((a: { title?: string }) => a.title).join('; ')}`,
                  },
                ],
              }),
            });
    ...
    for (const candidate of allCandidates) {
      ...
      const gf = candidate.geofence as
      ...
      for (const policy of policies ?? []) {
        ...
        if (
          pLat != null &&
          pLng != null &&
          !isWithinCircle(pLat, pLng, eventLat, eventLng, radiusKm)
        )
          continue;
        ...
        const [dupResult, rapidResult] = await Promise.all([
          supabase
            ...
            .select('id')
            .eq('policy_id', policy.id)
            .eq('disruption_event_id', event.id)
            .limit(1),
