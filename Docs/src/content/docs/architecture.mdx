...
```mermaid
flowchart TB
    UI["1. Rider picks a weekly plan"] --> CC["Create Stripe Checkout session"]
    CC --> SC["2. Stripe-hosted payment page"]
    SC --> WH["3. Secure webhook from Stripe"]
    WH --> WP["Store weekly coverage<br/>(weekly_policies)"]
    WH --> PT["Store payment record<br/>(payment_transactions)"]
    WH --> RD["Send rider back to dashboard<br/>with coverage active"]
```
...
| **Pricing engine** | Suggests a fair weekly price (₹49–₹199) based on **how risky a rider’s zone and history look**, while keeping plans simple to understand. |
