...
flowchart TB
    ...
    subgraph App["App Server (Next.js)"]
        ...
        API["API routes\n(payments · cron · admin)"]
        Logic["Core logic\n(trigger engine · fraud checks · pricing)"]
    end

    subgraph Data["Data & Services"]
        Supabase[Supabase]
        ExtAPI[External APIs\n(weather · AQI · traffic · news · payments · LLM)]
    end
...
flowchart TB
    ...
    WH --> WP["Store weekly coverage\n(weekly_policies)"]
    WH --> PT["Store payment record\n(payment_transactions)"]
    WH --> RD["Send rider back to dashboard\nwith coverage active"]
```
...
flowchart TB
    ...
    Z --> T["2. Ask APIs:\nIs weather / AQI / traffic / restriction bad enough?"]
    T --> DD["3. Group nearby events\n(avoid double-counting same disruption)"]

    DD --> Loop["4. For each real disruption"]
    Loop --> Evt["Record disruption\n(live_disruption_events)"]
    Loop --> Pol["Find covered riders\nin the affected area"]
    Loop --> Cap["Respect weekly caps\n(max claims per rider)"]
    Loop --> Fraud["Run fraud & sanity checks"]
    Loop --> Claims["Create auto-claims\n(parametric_claims)"]

    Claims --> Log["5. Log what happened\n(for audit & debugging)"]
```
