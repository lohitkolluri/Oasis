...
- Clean, minimal UI inspired by Linear/Vercel/Stripe; avoid generic “vibe-coded” gradients, noisy animations, and flashy “Active/Live” badges
...
- Avoid running builds/tests when the user explicitly requests “no build at the end”
...
- External APIs: Tomorrow.io (weather), WAQI (AQI), Open-Meteo (fallback), NewsData (disruptions), TomTom (traffic), OSRM (routing), Photon (typeahead geocoding), Nominatim (geocoding), DiceBear (avatars)
...
- Shell is `zsh`; quote paths containing parentheses (e.g. `app/(auth)/...`) or use `git add -- 'path'` to avoid glob expansion
