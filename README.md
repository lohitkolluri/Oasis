...
- **"Growth Mode" Black Swan Protection (Option B)** automatically absorbing financial hits during catastrophic cyclical events. Instead of squishing policies to match math limits, exposure is dynamically capped to 1 payout event/week, and absolute payouts are floored at `1.5x` the rider's **weekly** premium, prioritizing rider satisfaction gracefully subsidized by the platform's **annualized** business margin (LTV economics — **not** annual premium billing; pricing remains weekly per use case).
